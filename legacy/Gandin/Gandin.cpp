...
T pref_Orine; // single nucleation
/*Phase diagram*/
T T_Melt;      // K
T T_Eute;      // K
...
void readParam() {
  ...
  /*nucleation and growth*/
  Nuc_Dens_Surf = param_reader.getValue<T>("Nuc_and_Growth", "Nuc_Dens_Surf");
  Nuc_Dens_Bulk = param_reader.getValue<T>("Nuc_and_Growth", "Nuc_Dens_Bulk");
  DT_Mean_Surf = param_reader.getValue<T>("Nuc_and_Growth", "DT_Mean_Surf");
  DT_Std_Surf = param_reader.getValue<T>("Nuc_and_Growth", "DT_Std_Surf");
  DT_Mean_Bulk = param_reader.getValue<T>("Nuc_and_Growth", "DT_Mean_Bulk");
...
int main() {
  ...
  std::vector<BasicBoundary<T, LatSet0, LatStru0> *> NSBoundary{&NS_BB};
  ...
  // TH_ABB.SetFromGeo(1, Geo);
  std::vector<BasicBoundary<T, LatSet1, LatStru1> *> THBoundary{&TH_BB};
  BoundaryManager2D<T, LatSet1, LatStru1> TH_BM(THBoundary);
  ...
  lbm2D<T, LatSet1, LatStru1> TH(Field, TempConv, TH_BM, std::string("TH"),
                                 std::string("T"));
  // TH.setPopRho_From_VoxelFlag(1, T(0));
  ...
  // --------------------- CA ---------------------
  GandinCA2D<T, LatStruca> CA(CAField, CAConv, LBM);
  CA.SingleNuc(int(Ni / 2), int(Nj / 2), pref_Orine);
  ...
  datWriter.Write_GCA(MainLoopTimer(), CAField);
