...
struct Equilibrium {
  ...
  static inline T Order2(int k, const Vector<T, LatSet::d> &u, T rho, T u2) {
    ...
    //            (T(1) + LatSet::InvCs2 * (u * LatSet::c[k]) +
    // pow((u * LatSet::c[k]), 2) * T(0.5) * LatSet::InvCs4 -
  }

  // static inline T Order2_Incompresible(int k, const Vector<T, LatSet::d> &u, T rho, T u2) {
  //   T uc = u * LatSet::c[k];
  //   return LatSet::w[k] * (rho + LatSet::InvCs2 * uc + uc * uc * T(0.5) * LatSet::InvCs4 -
  //                          LatSet::InvCs2 * u2 * T(0.5));
  // }
  ...
  static void FirstOrder(std::array<T, LatSet::q> &feq, const Vector<T, LatSet::d> &u, T rho) {
    for (int k = 0; k < LatSet::q; ++k) {
      feq[k] = Order1(k, u, rho);
    }
  }
  ...
  static void FirstOrder_Incompresible(std::array<T, LatSet::q> &feq, const Vector<T, LatSet::d> &u,
                                       T rho) {
    for (int k = 0; k < LatSet::q; ++k) {
      feq[k] = Order1_Incompresible(k, u, rho);
    }
  ...
  static void SecondOrder(std::array<T, LatSet::q> &feq, const Vector<T, LatSet::d> &u, T rho) {
    T u2 = u.getnorm2();
    for (int k = 0; k < LatSet::q; ++k) {
      feq[k] = Order2(k, u, rho, u2);
    }
