...
#include "legacy/legacy_utils/legacy_util.h"
...
struct ZScell2D {
  ...
  // Vector2D<T> Vertice;
  ZScell2D() : Id(0), C_Solid(), deltaf(T(0)), K(T(0)) {}
  ZScell2D(int id) : Id(id), C_Solid(), deltaf(T(0)), K(T(0)) {}
...
class ZhuStefanescu2D : public FDM2D<T>, public LatStru {
 private:
  ...
  std::vector<std::vector<ZScell2D<T>>> Captureds_OMP;
...
#pragma omp parallel for
 ...
 public:
  ...
  void EraseInactive();
  void EraseInactive_s(std::vector<ZScell2D<T>>& cells);
  ...
  void Grow_erase();
  void Grow_erase_s(std::vector<ZScell2D<T>>& cells);

  // deal with newly solified cells serially
  void Solified_s(const std::vector<SolifiedZScell2D<T>>& solifieds);

  // capture scheme:
  // 1. simple capture:
  // capture cells which have solid neighbors (at least one out of 8)
  // the task is quite simple, parallelization may not improve performance
  void SimpleCapture();
  // void SimpleCapture_omp(std::vector<ZScell2D<T>>& captureds, int start,
  //                        int end);
  ...
  void TransformInterface() {
    ...
    for (auto& cell : Cells) {
      Interface.emplace_back(cell.Id);
    }
    // std::transform(Cells.begin(), Cells.end(), std::back_inserter(Interface),
  ...
  void getdeltaf(ZScell2D<T>& cell);
  ...
  inline bool all_NbrSolid(int id, int end = LatStru::q) {
    for (int i = 0; i < end; i++) {
      if (CA.State[id + LatStru::Nbr[i]] != -1) return false;
    }
    return true;
  }
